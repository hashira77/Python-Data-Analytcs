...
c2.metric("maximum immigration", f"{total_immig/1000000:.2f}M")
...
country = c1.selectbox("Select a couintry", countries)
...
figContinent = px.pie(cdf, x=cdf.index, y='total',
                    title="Contitnent wise Immigration", hole=.5)
c2.plotly_chart(figContinent, use_container_width=True)
figMap = px.choropleth(df, locations=df.index,
